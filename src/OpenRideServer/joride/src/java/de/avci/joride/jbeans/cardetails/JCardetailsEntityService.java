...
public class JCardetailsEntityService {
    ...
    CarDetailsControllerLocal carDetailsControllerBean = lookupCarDetailsControllerBeanLocal();
      ...
      /** Get a list of Cars (CarDetailEntity Objects) for this user.
       *  As always, user is determined savely from http auth data.
       * 
       * @return 
       */
      ...
      /** Savely retrieve the carDetailsEnttity with the given ID.
       *  Savely means:
       *  Determine the caller from HTTPRequest (AuthPrincipal)
       *  get the CarDetails Object by cardetID
       *  Check if the CarDetailsObject belongs to the Caller
       *  If so, return the cardetailsObject,
       *  else, throw an error.
       * 
       * @param cardetID 
       */
     public CarDetailsEntity getCarDetailsEntitySavely(int cardetId){
         ...
         if(ce==null){ throw new Error("Cannot retrieve customer from Request, Error thrown");}
      
         if(cde==null){ 
             System.err.println("Cannot retrieve car details for id "+cardetId+" returning null");
             return null;
         }
    ...
    /** Savely delete the cardetails object with the given cardetails ID.
     *  That is: 
     *  Determine the caller from http request,
     *  see, if cardet belongs to the caller,
    ...
    /** Write this Object savely to Database.
     *  That is: 
     *  Check that caller in http request is indeed the ownwer
     *  of this CardetEntity, 
     *  if so, then updateData, if not then fail with an error<
     * 
     * 
     * @param jcde 
     */
