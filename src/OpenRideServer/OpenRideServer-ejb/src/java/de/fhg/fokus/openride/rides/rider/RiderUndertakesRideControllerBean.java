 ...
 */
@Stateless
public class RiderUndertakesRideControllerBean extends ControllerBean implements
		RiderUndertakesRideControllerLocal {
	...
	public Point getStartPoint(int rideId) {
		...
		List<RiderUndertakesRideEntity> c = (List<RiderUndertakesRideEntity>) em
				.createNamedQuery("RiderUndertakesRideEntity.findByRideId")
	...
	public Timestamp getStartTime(int rideId) {
		...
		List<RiderUndertakesRideEntity> c = (List<RiderUndertakesRideEntity>) em
				...
				.setParameter("rideId", rideId).getResultList();
		if (c.size() == 1) {
			returnValue = new Timestamp(c.get(0).getStarttimeEarliest()
					.getTime());
		}
		commitUserTransaction();
		return returnValue;
	 ...
	 *
	 * @param riderRouteId
	 *            FIXME: this is the fake <code>rideID</code>
	 * @param riderrouteId
	 *            FIXME: this is the real rideId
	...
	public int addRiderToRide(int riderRouteId, int rideId) {
		...
		double sharedDistance = routeMatchingBean.computeAdaptedRoute(rideId,
				riderRouteId, driveRoutePoints, routePoints);

		if (sharedDistance != -1 && drive != null && ride != null
				&& ride.getRideId() == null) {
			// the ride is not yet booked
			driverUndertakesRideControllerBean.setRoutePoints(rideId,
					routePoints);
			driverUndertakesRideControllerBean.setDriveRoutePoints(rideId,
					driveRoutePoints);
		...
		List<MatchEntity> list1 = (List<MatchEntity>) em
				.createNamedQuery("MatchEntity.findByRiderrouteId")
				.setParameter("riderrouteId", riderRouteId).getResultList();
		// If the Rider accepts the ride. All other drivers must be informed
		// about that.
		for (MatchEntity matchEntity : list1) {
			if (matchEntity != null
					&& (matchEntity.getMatchEntityPK().getRideId() != rideId)) {
		...
		// clean up database
		List<MatchEntity> matches = driverUndertakesRideControllerBean
				.getMatches(rideId, true);
		List<MatchEntity> potentiallyOutdatedMatches = new ArrayList<MatchEntity>();
		for (MatchEntity match : matches) {
			if (match.getDriverState() != null && match.getRiderState() != null
					&& match.getDriverState().equals(MatchEntity.ACCEPTED)
					&& match.getRiderState().equals(MatchEntity.ACCEPTED)) {
				...
				// (tku 01/09/10): don't remove yet, will check if it is
				// outdated in next step:
				potentiallyOutdatedMatches.add(match);
				// em.remove(match);
			}
		}
		// search for new Matches!
		List<MatchEntity> newMatches = routeMatchingBean
				.searchForRiders(rideId);
		...
		for (MatchEntity poM : potentiallyOutdatedMatches) {
			...
			for (MatchEntity newM : newMatches) {
				if (poM.getMatchEntityPK().getRideId() == newM
						.getMatchEntityPK().getRideId()
						&& poM.getMatchEntityPK().getRiderrouteId() == newM
								.getMatchEntityPK().getRiderrouteId()) {
					// Don't remove if rideId and riderRouteId match those of
					// the new match (new match will be filtered in next step)
					isOutdated = false;
					break;
				}
			}
			if (isOutdated) {
				...
				// TODO: set state to NO_MORE_AVAILABLE instead of deleting if
				// riderState was ACCEPTED... but need to discuss/think about
	...
	private void callMatchingAlgorithm(int riderrouteId, boolean setRiderAccess) {
		...
		List<MatchEntity> matches = routeMatchingBean
				.searchForDrivers(riderrouteId);
		...
		for (MatchEntity m : matches) {
			...
			CustomerEntity driver = m.getDriverUndertakesRideEntity()
					.getCustId();
	...
	public void setReceivedRating(int riderRouteId, int rating,
			String ratingComment) {
		RiderUndertakesRideEntity r = em.find(RiderUndertakesRideEntity.class,
				riderRouteId);
		if (r != null) {
	...
	public void setGivenRating(int riderRouteId, int rating,
			String ratingComment) {
		RiderUndertakesRideEntity r = em.find(RiderUndertakesRideEntity.class,
				riderRouteId);
		if (r != null) {
			...
			System.err.println("Setting givenRating : " + "  riderrouteId  : "
					+ riderRouteId + ", rating : " + rating + ", comment: "
					+ ratingComment);
	...
	public List<RiderUndertakesRideEntity> getRidesByCustId(String custId) {
		...
		returnList = em
				.createNamedQuery("RiderUndertakesRideEntity.findByCustId")
	 ...
	 *
	 *
	 *         TODO: when looking inside the code found in original OpenRide, I
	 *         found that contrary to what the method's name indicates, and
	 *         contrary to the docs this method returns *future* rides only.
	 *         Since it is not quite cleare where this is used, I have left it
	 *         in place unchanged, but added getAllRidesForCustomer() method to
	 *         do what was originally indicated.
	 *
	 */
	public List<RiderUndertakesRideEntity> getRides(String nickname) {
		...
		List<CustomerEntity> c = (List<CustomerEntity>) em
				.createNamedQuery("CustomerEntity.findByCustNickname")
				.setParameter("custNickname", nickname).getResultList();

		if (c.size() == 1) {
			// Get the Entity. It should be only one, since nicknames are unique
			// by constraint.
			...
			returnList = em
					.createNamedQuery("RiderUndertakesRideEntity.findByCustId")
					.setParameter("custId", e).getResultList();
		}
		if (returnList == null) {
			...
			Collections.sort(future, new Comparator() {
				public int compare(Object o1, Object o2) {
					...
					if (ent1 == null || ent2 == null) {
						return 0;
					} else if (ent1.getStarttimeEarliest().before(
							ent2.getStarttimeEarliest())) {
						return -1;
					} else if (ent1.getStarttimeEarliest().after(
							ent2.getStarttimeEarliest())) {
						return 1;
					} else {
						return 0;
	...
	public List<RiderUndertakesRideEntity> getRidesForCustomer(CustomerEntity ce) {
		...
		returnList = em
				.createNamedQuery("RiderUndertakesRideEntity.findByCustId")
				.setParameter("custId", ce).getResultList();
	...
	public List<RiderUndertakesRideEntity> getInactiveRideRequests(
			String nickname) {
		List<RiderUndertakesRideEntity> returnList = null;
		List<RiderUndertakesRideEntity> past = new ArrayList<RiderUndertakesRideEntity>();
		List<RiderUndertakesRideEntity> future = new ArrayList<RiderUndertakesRideEntity>();
		...
		// Get the CustomerEntity by <code>nickname</code>
		CustomerEntity c = (CustomerEntity) em
				.createNamedQuery("CustomerEntity.findByCustNickname")
				.setParameter("custNickname", nickname).getSingleResult();
		...
		if (c != null) {
			...
			returnList = em
					.createNamedQuery("RiderUndertakesRideEntity.findByCustId")
					.setParameter("custId", c).getResultList();
			// Use the Id of the user to get his rides.
			// TODO: Use Franks tables which contain the active rides to find
		...
		if (returnList == null) {
			...
			now.setTime(now.getTime()
					- DriverUndertakesRideControllerBean.ACTIVE_DELAY_TIME);
			for (RiderUndertakesRideEntity d : returnList) {
	...
	public List<RiderUndertakesRideEntity> getActiveRideRequests(String nickname) {
		...
		if (returnList == null) {
			...
			earlier.setTime(earlier.getTime()
					- DriverUndertakesRideControllerBean.ACTIVE_DELAY_TIME);
			for (RiderUndertakesRideEntity d : returnList) {
	...
	public RiderUndertakesRideEntity getRideByRiderRouteId(int riderRouteId) {
		...
		try {
			Query q = em
					.createNamedQuery("RiderUndertakesRideEntity.findByRiderrouteId");
	...
	public LinkedList<RiderUndertakesRideEntity> getAllRides() {
		...
		List<RiderUndertakesRideEntity> l = em.createNamedQuery(
				"RiderUndertakesRideEntity.findAll").getResultList();
		LinkedList<RiderUndertakesRideEntity> ll = new LinkedList<RiderUndertakesRideEntity>(
				l);
	...
	public List<RiderUndertakesRideEntity> getActiveRideRequestsByCustId(
			String custId) {
		List<CustomerEntity> e = (List<CustomerEntity>) em
				.createNamedQuery("CustomerEntity.findByCustId")
				.setParameter("custId", Integer.parseInt(custId))
				.getResultList();
		...
		if (e.size() > 0) {
			...
			List<RiderUndertakesRideEntity> c = (List<RiderUndertakesRideEntity>) em
					.createNamedQuery("RiderUndertakesRideEntity.findByCustId")
					.setParameter("custId", en).getResultList();
			return c;
		} else {
	 ...
	 */
	public List<RiderUndertakesRideEntity> getRatedRidesByRider(
			CustomerEntity rider) {
		try {
			List<RiderUndertakesRideEntity> entities = em
					.createNamedQuery(
							"RiderUndertakesRideEntity.findRatedRidesByRider")
					.setParameter("custId", rider).getResultList();
			return entities;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	 ...
	 */
	public List<RiderUndertakesRideEntity> getRatedRidesByDriver(
			CustomerEntity driver) {
		try {
			List<RiderUndertakesRideEntity> entities = em
					.createNamedQuery(
							"RiderUndertakesRideEntity.findRatedRidesByDriver")
					.setParameter("custId", driver).getResultList();
			return entities;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
	...
	public int getRatingsTotalByCustomer(CustomerEntity customer) {
		int ratingsTotal = 0;
		try {
			Object ratingsAsRider = em
					.createNamedQuery(
							"RiderUndertakesRideEntity.sumUpRatingsAsRider")
					.setParameter("custId", customer).getSingleResult();
			if (ratingsAsRider != null) {
				ratingsTotal += Integer.parseInt(ratingsAsRider.toString());
			}
			Object ratingsAsDriver = em
					.createNamedQuery(
							"RiderUndertakesRideEntity.sumUpRatingsAsDriver")
					.setParameter("custId", customer).getSingleResult();
		...
		// System.out.println("ratingsTotal for customer "+
		// customer.getCustNickname() +": "+ratingsTotal);
		return ratingsTotal;
	}

	public float getRatingsRatioByCustomerAndDate(CustomerEntity customer,
			Date fromDate) {
		...
		try {
			...
			positiveRatings = Integer
					.parseInt(em
							.createNamedQuery(
									"RiderUndertakesRideEntity.countPositiveRatingsAsRider")
			...
			positiveRatings += Integer
					.parseInt(em
							.createNamedQuery(
									"RiderUndertakesRideEntity.countPositiveRatingsAsDriver")
							.setParameter("custId", customer)
							.setParameter("fromDate", fromDate)
							.getSingleResult().toString());
			// neutralRatings =
			// Integer.parseInt(em.createNamedQuery("RiderUndertakesRideEntity.countNeutralRatingsAsRider").setParameter("custId",
			// customer).setParameter("fromDate",
			// fromDate).getSingleResult().toString());
			// neutralRatings +=
			// Integer.parseInt(em.createNamedQuery("RiderUndertakesRideEntity.countNeutralRatingsAsDriver").setParameter("custId",
			// customer).setParameter("fromDate",
			// fromDate).getSingleResult().toString());
			negativeRatings = Integer
					.parseInt(em
							.createNamedQuery(
									"RiderUndertakesRideEntity.countNegativeRatingsAsRider")
							.setParameter("custId", customer)
							.setParameter("fromDate", fromDate)
							.getSingleResult().toString());
			negativeRatings += Integer
					.parseInt(em
							.createNamedQuery(
									"RiderUndertakesRideEntity.countNegativeRatingsAsDriver")
							.setParameter("custId", customer)
							.setParameter("fromDate", fromDate)
							.getSingleResult().toString());
			if (positiveRatings == 0) {
				...
				ratingsRatio = (float) positiveRatings
						/ (float) (positiveRatings + /*
													 * neutralRatings +
													 */negativeRatings);
			}
		} catch (Exception e) {
		...
		// System.out.println("ratingsRatio for customer "+
		// customer.getCustNickname() +": "+ratingsRatio);
		return ratingsRatio;
	}
	...
	public int getPositiveRatingsTotalByCustomerAndDate(
			CustomerEntity customer, Date fromDate) {
	...
	public int getNeutralRatingsTotalByCustomerAndDate(CustomerEntity customer,
			Date fromDate) {
		...
		try {
			...
			neutralRatings = Integer
					.parseInt(em
							.createNamedQuery(
									"RiderUndertakesRideEntity.countNeutralRatingsAsRider")
							.setParameter("custId", customer)
							.setParameter("fromDate", fromDate)
							.getSingleResult().toString());
			neutralRatings += Integer
					.parseInt(em
							.createNamedQuery(
									"RiderUndertakesRideEntity.countNeutralRatingsAsDriver")
							.setParameter("custId", customer)
							.setParameter("fromDate", fromDate)
							.getSingleResult().toString());
		} catch (Exception e) {
	...
	public int getNegativeRatingsTotalByCustomerAndDate(
			CustomerEntity customer, Date fromDate) {
	...
	public boolean isRemovable(int riderrouteId) {
		...
		startUserTransaction();
		List<MatchEntity> states = (List<MatchEntity>) em
				.createNamedQuery("MatchEntity.findByRiderrouteId")
				.setParameter("riderrouteId", riderrouteId).getResultList();
		...
		if (states.size() > 0) {
			...
			for (MatchEntity entity : states) {
				if (entity.getDriverState() != null
						|| entity.getRiderState() != null) {
					deletable = false;
				}
			}
		}
	...
	private boolean removeRideCompletely(int riderrouteId) {
		...
		List<RiderUndertakesRideEntity> entity = em
				.createNamedQuery(
						"RiderUndertakesRideEntity.findByRiderrouteId")
				.setParameter("riderrouteId", riderrouteId).getResultList();
	...
	private boolean removeRideCountermandOnly(int riderrouteId) {
		...
		log.info("remove (countermand only) ride request riderrouteId :"
				+ riderrouteId);
		...
		List<RiderUndertakesRideEntity> rides = em
				.createNamedQuery(
						"RiderUndertakesRideEntity.findByRiderrouteId")
				.setParameter("riderrouteId", riderrouteId).getResultList();
		...
		commitUserTransaction();
		log.info("removed (countermand only) ride request riderrouteId :"
				+ riderrouteId);
		return false;
	}
	...
	public int addRideRequest(int cust_id, Date starttime_earliest,
			...
			double price, String comment, String startptAddress,
	...
	public void removeRiderFromRide(int riderrouteid, int rideid) {
		...
		if (riderundertakesride.getRideId() != null) {
			// FIXME: just a test!
			List<RiderUndertakesRideEntity> rider = driverUndertakesRideControllerBean
					.getRidersForDrive(rideid);
			int numOfRiders = rider.size();

			// mark match as countermanded
			List<MatchEntity> states = (List<MatchEntity>) em
					.createNamedQuery("MatchEntity.findByRideIdRiderrouteId")
					.setParameter("riderrouteId", riderrouteid)
					.setParameter("rideId", rideid).getResultList();
			...
			for (Iterator<MatchEntity> it = states.iterator(); it.hasNext();) {
				...
				matchEntity.setRiderChange(new Date());
				// Notify the driver:
				CustomerEntity driver = matchEntity
						.getDriverUndertakesRideEntity().getCustId();
				driver.updateCustLastMatchingChange();
				em.merge(driver);
				em.merge(matchEntity);
			}
			...
			logger.log(Level.INFO,
					numOfRiders
							+ " die neue Size: "
							+ driverUndertakesRideControllerBean
									.getRidersForDrive(rideid).size());
		} else {
	...
	public List<MatchEntity> getMatches(int riderrouteId, boolean setRiderAccess) {
		...
		List<MatchEntity> list1 = (List<MatchEntity>) em
				...
				.setParameter("riderrouteId", riderrouteId).getResultList();
		RiderUndertakesRideEntity ride = getRideByRiderRouteId(riderrouteId);
		List<RiderUndertakesRideEntity> drives = getInactiveRideRequests(ride
				.getCustId().getCustNickname());
		if (drives.contains(ride)) {
			// The ride is already inactive so just return such matches which
			// are already completed
			List<MatchEntity> entities = (List<MatchEntity>) em
					.createNamedQuery(
							"MatchEntity.findByRiderrouteIdSuccessful")
					.setParameter("riderrouteId", riderrouteId).getResultList();
			newMatches = entities;
			...
			for (MatchEntity m : list1) {
				// List<DriverUndertakesRideEntity> driveList = new
				// ArrayList<DriverUndertakesRideEntity>();
				if ((m.getDriverState() != null && m.getRiderState() != null)
						&& (m.getDriverState().equals(MatchEntity.ACCEPTED) && m
								.getRiderState().equals(MatchEntity.ACCEPTED))) {
					// If there already exists one Match that has been finished
					 ...
					 */
				} else if ((m.getDriverState() != null && m.getDriverState()
						.equals(MatchEntity.REJECTED))
						|| (m.getRiderState() != null && m.getRiderState()
								.equals(MatchEntity.REJECTED))) {
					...
					// seats =
					// driverUndertakesRideControllerBean.getDriveByDriveId(rideid).getRideOfferedseatsNo();
					// DriverUndertakesRideEntity ride =
					// driverUndertakesRideControllerBean.getDriveByDriveId(rideid);
					// List<RiderUndertakesRideEntity> drives =
					// (List<RiderUndertakesRideEntity>)
					// em.createNamedQuery("RiderUndertakesRideEntity.findByRideId").setParameter("rideId",
					// ride).getResultList();
					// if (drives.size() >= seats) {
					// this match cannot be added, because the drive is already
					// complete
					// } else {
					// FIXME: this could be a problem if bookings are made while
					// a user is sending his acceptance! How can we find out
					// whether a booking is still ok?
					newMatches.add(m);
					// }
				}

				if (setRiderAccess) {
					// Mark all possible matches as accessed (otherwise, status
					// update would be displayed indefinitely)
					m.setRiderAccess(new Date());
					em.merge(m);
				}
		 ...
		 * 
		 * //persist open matchings newMatches.addAll(matches);
		 */
		newMatches.addAll(rejectedMatches);

		if (list.size() > 0) { // tku: Can there ever be more than 1 match in
								// "list"???
			Collections.sort(list, new Comparator() {
				public int compare(Object o1, Object o2) {
					...
					if (o1 == null || o2 == null) {
						...
						return new Integer(ent1.getMatchEntityPK().getRideId())
								.compareTo(ent2.getMatchEntityPK().getRideId());
					}
			...
			// Limit number of unrejected matches to return to `matchCountLimit`
			// (tku 23/08/10)
			...
			if (newMatches.size() > matchCountLimit) {
				...
				for (MatchEntity m : newMatches) {
					// Generally don't remove matches with states 0, 2, 3, 4
					// (rejected, countermanded, no more available),
					...
					if (!(m.getDriverState() == MatchEntity.ACCEPTED
							...
							|| m.getDriverState() == MatchEntity.DRIVER_COUNTERMANDED || m
							.getDriverState() == MatchEntity.NO_MORE_AVAILABLE)
							&& !(m.getRiderState() == MatchEntity.ACCEPTED && m
									.getDriverState() == MatchEntity.ACCEPTED)) {
				...
				logger.log(
						Level.INFO,
						"RURC.getMatches: "
								+ (prevSize - newMatches.size())
								+ " of "
								+ prevSize
								+ " matches dropped (limiting unrejected machtes to 3).");
			}
	...
	private List<MatchEntity> filter(List<MatchEntity> matches) {
		...
		if (matches != null) {
			for (Iterator<MatchEntity> it = matches.iterator(); it.hasNext();) {
				...
				if (em.createNamedQuery("MatchEntity.findByRideIdRiderrouteId")
						.setParameter("rideId",
								matchEntity.getMatchEntityPK().getRideId())
						.setParameter(
								"riderrouteId",
								matchEntity.getMatchEntityPK()
										.getRiderrouteId()).getResultList()
						.size() > 0) {
	...
	public boolean isRideUpdated(int riderrouteId) {
		if (em.createNamedQuery(
				"MatchEntity.findChangesSinceAccessByRiderByRiderrouteId")
				.setParameter("riderrouteId", riderrouteId).getResultList()
				.size() > 0) {
			return true;
		} else {
	...
	public void setMatchCountermand(Integer rideId, Integer riderrouteId) {
		...
		if (match.size() > 0) {
			...
			CustomerEntity driver = match.get(0)
					.getDriverUndertakesRideEntity().getCustId();
			driver.updateCustLastMatchingChange();
	...
	public boolean invalidateRide(Integer riderrouteId) {
		...
		RiderUndertakesRideEntity rue = this
		...
		Date latency = new Date(System.currentTimeMillis()
				- (1000 * 60 * 60 * 24));
		...
		LinkedList<MatchEntity> matchList = routeMatchingBean
				.searchForDrivers(riderrouteId);
		...
		for (MatchEntity match : matchList) {
			...
			// notify riders
			CustomerEntity rider = match.getRiderUndertakesRideEntity()
					.getCustId();
			rider.updateCustLastMatchingChange();
	...
	@Override
	public List<RiderUndertakesRideEntity> getRidesForCustomer(
			CustomerEntity ce, Date startDate, Date endDate) {

		List<RiderUndertakesRideEntity> res = em
				.createNamedQuery(
						"RiderUndertakesRideEntity.findByRidersRidesBetween")
				.setParameter("custId", ce)
	...
	@Override
	public List<RiderUndertakesRideEntity> getRealizedRidesForRider(
			CustomerEntity ce, Date startDate, Date endDate) {

		List<RiderUndertakesRideEntity> res = em
				.createNamedQuery(
						"RiderUndertakesRideEntity.findByRidersRealizedRidesBetween")
				.setParameter("custId", ce)
				.setParameter("startDate", startDate)
				.setParameter("endDate", endDate).getResultList();
		return res;
	...
	@Override
	public List<RiderUndertakesRideEntity> getUnratedRidesForRider(
			CustomerEntity ce, Date startDate, Date endDate) {

		List<RiderUndertakesRideEntity> res = em
				.createNamedQuery(
						"RiderUndertakesRideEntity.findByRidersUnratedRidesBetween")
				.setParameter("custId", ce)
				.setParameter("startDate", startDate)
				.setParameter("endDate", endDate).getResultList();
		return res;
	...
	@Override
	public List<RiderUndertakesRideEntity> getUnratedRidesForDriver(
			CustomerEntity ce, Date startDate, Date endDate) {

		List<RiderUndertakesRideEntity> res = em
				.createNamedQuery(
						"RiderUndertakesRideEntity.findByDriversUnratedRidesBetween")
				.setParameter("custId", ce)
				.setParameter("startDate", startDate)
				.setParameter("endDate", endDate).getResultList();
		return res;
	...
	@Override
	public List<RiderUndertakesRideEntity> getRidesForDriver(CustomerEntity ce,
			Date startDate, Date endDate) {

		// TODO: setting start date, end date currently omitted
		List<RiderUndertakesRideEntity> res = em
				.createNamedQuery(
						"RiderUndertakesRideEntity.findByDriversRidesBetween")
				.setParameter("custId", ce)
				.setParameter("startDate", startDate)
				.setParameter("endDate", endDate).getResultList();
		return res;
	...
	public Integer getCountOfRatingsForDriver(CustomerEntity customer) {
		...
		try {
			Object ratingsAsDriver = em
					.createNamedQuery(
							"RiderUndertakesRideEntity.countRatingsAsDriver")
					.setParameter("custId", customer).getSingleResult();
			if (ratingsAsDriver != null) {
				res = Integer.parseInt(ratingsAsDriver.toString());
			}
	...
	public Integer getCountOfRatingsForRider(CustomerEntity customer) {
		...
		try {
			Object ratingsAsRider = em
					.createNamedQuery(
							"RiderUndertakesRideEntity.countRatingsAsRider")
					.setParameter("custId", customer).getSingleResult();
			if (ratingsAsRider != null) {
				res = Integer.parseInt(ratingsAsRider.toString());
			}
	...
	@Override
	public List<RiderUndertakesRideEntity> getRidesForRiderAfterDate(
			CustomerEntity ce, Date startDate) {

		Query query = em
				.createNamedQuery("RiderUndertakesRideEntity.findRidesAfterDateforCustId");
		List<RiderUndertakesRideEntity> res = query.setParameter("custId", ce)
	 ...
	 *
	 * @param rideId
	 *            rideId of associated riderundertakesrideEntity
	 * @param riderrouteId
	 *            riderrouteId of associated driverundertakesrideEntity
	 * @return
	 */
	private MatchEntity fetchMatchEntity(Integer rideId, Integer riderrouteId) {

		MatchEntity me = (MatchEntity) em
				.createNamedQuery("MatchEntity.findByRiderIdAndRiderrouteId")
				.setParameter("rideId", rideId)
				.setParameter("riderrouteId", riderrouteId).getSingleResult();
	...
	public Integer noOfOpenRequests(Integer custId) {
		...
		// @NamedQuery(name =
		// "RiderUndertakesRideEntity.countRidesAfterDateforCustId", query =
		// "SELECT count(r.riderrouteId) FROM RiderUndertakesRideEntity r WHERE r.custId = :custId  AND (r.starttimeLatest >= :startDate) ORDER BY r.starttimeLatest"),
		Long resLong = (Long) em
				.createNamedQuery(
						"RiderUndertakesRideEntity.countRidesAfterDateforCustId")
				.setParameter("custId", customer)
	...
	public Integer noOfLeftRequests(Integer customerId) {
		...
		CustomerEntity customer = customerControllerBean
