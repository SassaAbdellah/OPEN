...
public class JRiderUndertakesRideEntityService {
	...
	public RiderUndertakesRideControllerLocal lookupRiderUndertakesRideControllerBeanLocal() {
		try {
			...
			Object o = c
					.lookup("java:global/OpenRideServer/OpenRideServer-ejb/RiderUndertakesRideControllerBean!de.fhg.fokus.openride.rides.rider.RiderUndertakesRideControllerLocal");
			if (o == null) {
				throw new Error(
						"Error while looking up RiderUndertakesRideControllerLocal, lookup result is null. ");
			}
	...
	public List<JRiderUndertakesRideEntity> getRidesForRiderInInterval() {
		...
		RideSearchParamsBean tb = new RideSearchParamsBean()
				.retrieveCurrentTimeInterval(param);

		log.log(Level.WARNING,
				"Updated Time Interval " + tb.getStartDateFormatted() + " -> "
						+ tb.getEndDateFormatted());

		// get all rides related to this customer
		List<RiderUndertakesRideEntity> res1 = rurcl.getRidesForCustomer(ce,
				tb.getStartDate(), tb.getEndDate());
	...
	public List<JRiderUndertakesRideEntity> getRidesForDriverInInterval() {
		...
		// get all rides related to this customer
		List<RiderUndertakesRideEntity> res1 = rurcl.getRidesForDriver(ce,
				tb.getStartDate(), tb.getEndDate());
		...
		log.log(Level.FINE, "" + this.getClass()
				+ "getRidesForDriverInInterval returning " + res.size()
				+ " results");
	...
	public List<JRiderUndertakesRideEntity> getRealizedRidesForRiderInInterval() {
		...
		// get all rides related to this customer
		List<RiderUndertakesRideEntity> res1 = rurcl.getRealizedRidesForRider(
				ce, tb.getStartDate(), tb.getEndDate());
	...
	/**
	 * Get a list of all rides for the actual Rider in that have starttime
	 * latest after NOW
	 *
	 *
	 *
	...
	public List<JRiderUndertakesRideEntity> getFutureRidesForRider() {
		...
		// get all rides related to this customer
		List<RiderUndertakesRideEntity> res1 = rurcl.getRidesForRiderAfterDate(
				ce, new Date());
	 ...
	 * 
	 * @param jrfl
	 *            typically empty object to be initiallized by this method
	 */
	...
	/**
	 * List unrated rides for Rider with pickup time earlier then now
	 * (System.currentTimeMillis())
	 */
	public List<JRiderUndertakesRideEntity> getUnratedRidesForRider() {

		return this.getUnratedRidesForRiderInInterval(new Date(0), new Date(
				System.currentTimeMillis()));
	}
	...
	public List<JRiderUndertakesRideEntity> getUnratedRidesForRiderInInterval() {
		...
		return getUnratedRidesForRiderInInterval(tb.getStartDate(),
				tb.getEndDate());
	...
	private List<JRiderUndertakesRideEntity> getUnratedRidesForRiderInInterval(
		...
		// get all rides related to this customer
		List<RiderUndertakesRideEntity> res1 = rurcl.getUnratedRidesForRider(
	...
	/**
	 * List unrated rides for Driver with pickup time earlier then now
	 * (System.currentTimeMillis())
	 */
	public List<JRiderUndertakesRideEntity> getUnratedRidesForDriver() {

		return this.getUnratedRidesForDriverInInterval(new Date(0), new Date(
				System.currentTimeMillis()));
	}
	...
	public List<JRiderUndertakesRideEntity> getUnratedRidesForDriverInInterval() {
		...
		return getUnratedRidesForDriverInInterval(tb.getStartDate(),
				tb.getEndDate());
	}
	 ...
	 * @return
	 */
	private List<JRiderUndertakesRideEntity> getUnratedRidesForDriverInInterval(
			Date startDate, Date endDate) {
		...
		List<RiderUndertakesRideEntity> res1 = rurcl.getUnratedRidesForDriver(
				ce, startDate, endDate);
	 ...
	 * @return
	 */
	public RiderUndertakesRideEntity getRideByRiderRouteIdSavely(
			int myRiderRouteId) {
		...
		RiderUndertakesRideEntity rure = rurcl
				.getRideByRiderRouteId(myRiderRouteId);
	 ...
	 *            database
	 */
	public void updateJRiderUndertakesRideEntityByRiderRouteIDSavely(
			Integer riderRouteId, JRiderUndertakesRideEntity jrure) {
		...
		RiderUndertakesRideEntity rure = this
				.getRideByRiderRouteIdSavely(riderRouteId);
		// update
		jrure.updateFromRiderUndertakesRideEntity(rure);
	}
	...
	public List<RiderUndertakesRideEntity> getActiveOpenRides() {
		...
		if (ce.getCustNickname() == null) {
			throw new Error(
					"Cannot determine Rides, customer's nickname is null");
		}
	...
	public void setGivenRatingSavely(JRiderUndertakesRideEntity jrure) {
		...
		log.log(Level.FINE,
				"Set givenRating for RiderrouteId : " + jrure.getRiderrouteId()
						+ " Givenrating : " + jrure.getGivenrating()
						+ " GivenRatingComment : "
		...
		if (!(this.callerIsRider(jrure.getRiderrouteId()))) {
			throw new Error(
					"Cannot rate ride, caller is not identical to owner of ride request!");
		}

		rurcl.setGivenRating(jrure.getRiderrouteId(), jrure.getGivenrating(),
				jrure.getGivenratingComment());
	...
	public void setReceivedRatingSavely(JRiderUndertakesRideEntity jrure) {
		...
		log.log(Level.FINE,
				"Set receivedRating for RiderrouteId : "
						+ jrure.getRiderrouteId() + " Receivedrating : "
		...
		if (!(this.callerIsDriver(jrure.getRiderrouteId()))) {
			throw new Error(
					"Cannot driver-rate ride, caller is not identical to driver of ride request!");
		}

		rurcl.setReceivedRating(jrure.getRiderrouteId(),
				jrure.getReceivedrating(), jrure.getReceivedratingComment());
	...
	public boolean removeRideSafely(JRiderUndertakesRideEntity jrure) {
		...
		if (ce == null) {
			throw new Error(
					"Cannot determine Ride for removal, calling customerEntity is null");
		}
		...
		if (rurcl == null) {
			throw new Error(
					"Cannot determine RiderUndertakesRideControllerLocal");
		}
		...
		if (rure == null) {
			throw new Error("Cannot remove ride with id "
					+ jrure.getRiderrouteId() + ", ride is null!");
		}

		if (rure.getCustId().getCustId() != ce.getCustId()) {
			throw new Error(
					"Cannot retrieve Ride with given ID for removal, object does not belong to user");
		}
	...
	public boolean invalidateRequestSavely(int riderrouteId) {
		...
		RiderUndertakesRideControllerLocal rurcl = this
				.lookupRiderUndertakesRideControllerBeanLocal();
		RiderUndertakesRideEntity rue = rurcl
				.getRideByRiderRouteId(riderrouteId);
		...
		if (ce.getCustId() != rue.getCustId().getCustId()) {
			throw new Error(
					"Attempt to invalidate request that is not owned by User");
		}
		...
		if (rurcl.isRemovable(riderrouteId)) {
			...
			log.info("Request " + riderrouteId
					+ " is not deleteable, invalidating");
	...
	public boolean callerIsRider(int riderrouteId) {
		...
		if (null == rue) {
			throw new Error(
					"Cannot determine rider property, RiderUndertakesRideEntity is null");
		}

		if (null == rue.getCustId()) {
			throw new Error(
					"Cannot determine rider property, RiderUndertakesRideEntity has no custId");
		}
	...
	/**
	 * Return true, if caller currently can issue another request.
	 * 
	 * @return
	 */
	public Boolean canAddRequest() {
		...
		RiderUndertakesRideControllerLocal rurcl = this
				.lookupRiderUndertakesRideControllerBeanLocal();
		return rurcl.canAddRequest(ce.getCustId());
	}
