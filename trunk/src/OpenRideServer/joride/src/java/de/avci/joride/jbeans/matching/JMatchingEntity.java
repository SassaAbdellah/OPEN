...
public class JMatchingEntity implements Serializable {
	...
	public JRiderUndertakesRideEntity getRide() {
		...
		jride.updateFromRiderUndertakesRideEntity(matchEntity
				.getRiderUndertakesRideEntity());
		return jride;
	}
	...
	public JDriverUndertakesRideEntity getDrive() {
		...
		jdrive.updateFromDriverUndertakesRideEntity(matchEntity
				.getDriverUndertakesRideEntity());
	...
	/**
	 * True, if driver can either accept or reject this match (i.e: wether or
	 * not link should be shown in driveMatchingsList)
	 */
	...
	/**
	 * True, if rider can either accept or reject this match (i.e: wether or not
	 * link should be shown in rideMatchingsList)
	 */
	...
	public void smartUpdate() {
		...
		String riderrouteIdStr = hru
				.getParameterSingleValue(PARAM_NAME_riderrouteId);
		Integer riderrouteIdArg = new Integer(riderrouteIdStr);

		MatchEntity me = new JMatchingEntityService().getMatchSafely(rideIdArg,
				riderrouteIdArg);
	...
	public void setRiderMessage(String message) {
		new JMatchingEntityService().setRiderMessageSafely(
				this.getMatchEntity(), message);
	}
	...
	public void setDriverMessage(String message) {
		new JMatchingEntityService().setDriverMessageSafely(
				this.getMatchEntity(), message);
	}

	/**
	 * return true, if either rider or driver countermanded this ride, else
	 * false
	 * 
	 * @return true, if either rider or driver countermanded this ride, else
	 *         false
	...
	/**
	 * return true, if neither rider nor driver countermanded this ride, else
	 * false
	 * 
	 * @return true, if neither rider mor driver countermanded this ride, else
	 *         false
	 */
	...
	public String getMatchExpectedStartTimeFormatted() {
		...
		if (this.getMatchEntity() == null)
			return "--";
		if (this.getMatchEntity().getMatchExpectedStartTime() == null)
			return "--";
		return getDatetimeFormatted(this.getMatchEntity()
				.getMatchExpectedStartTime());
	}

	/**
	 * Send message from Driver to Rider, where message payload is the "message"
	 * property.
	 * 
	 * 
	 */
	...
	/**
	 * Send message from Rider to Driver, where message payload is the "message"
	 * property.
	 * 
	 * 
	 */
	...
	/**
	 * Get shared route in drivers preferred format
	 */
	...
	/**
	 * return false, if neither rider or driver countermanded this ride, else
	 * false
	 * 
	 * @return true, if either rider or driver countermanded this ride, else
	 *         false
	...
	public String getDebugPrintout() {
		...
		buf.append("RideId from PK        :"
				+ this.getMatchEntity().getMatchEntityPK().getRideId() + "\n");
		buf.append("RiderouteId from PK   :"
				+ this.getMatchEntity().getMatchEntityPK().getRiderrouteId()
				+ "\n");
		buf.append("=== Drive ==\n");
		buf.append("RideId from Drive     : " + this.getDrive().getRideId()
				+ "\n");
		buf.append("=== Ride ==\n");
		buf.append("RiderrouteId from Ride: "
				+ this.getRide().getRiderrouteId() + "\n");
		buf.append("============\n");
		return buf.toString();
	}
} // class
