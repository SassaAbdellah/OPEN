...
/**
 * Implementation of the CombiSearchAlgorithmORS providing access to the
 * carthesian-coordinate version of the ORS preselection subsystem.
 * 
 * I.e:
 * 
...
public class CombiSearchAlgorithmORSCarthesian extends CombiSearchAlgorithmORS {
	...
	/**
	 * String containing name of the sfdFunction, to be used in Prepared
	 * Statement
	 */
	private static String sfd = null;

	/**
	 * String containing name of the sfrFunction, to be used in Prepared
	 * Statement
	 */
	...
	static {
		...
		sfd = PropertiesLoader.getOperationalProperties().getProperty(
				PROP_NAME_SFD);
		if (sfd == null) {
			sfd = SFD_DEFAULT;
		}
		if (sfd.startsWith("$")) {
			sfd = SFD_DEFAULT;
		}// may happen if property is not properly initialized

		sfr = PropertiesLoader.getOperationalProperties().getProperty(
				PROP_NAME_SFR);
		if (sfr == null) {
			sfr = SFR_DEFAULT;
		}
	 ...
	 */
	private static final String preparedStatementSFDString = "select * from "
			+ sfd + "( ? )";
	...
	protected String getPreparedStatementSFDString() {
		if (MatchingLogger.canLogInfo()) {
			MatchingLogger
					.info("returning SFD Statement for carthesian Coordinates : "
							+ preparedStatementSFDString);
		}
	 ...
	 */
	private static final String preparedStatementSFRString = "select * from "
			+ sfr + "( ? )";
	...
	protected String getPreparedStatementSFRString() {
		if (MatchingLogger.canLogInfo()) {
			MatchingLogger
					.info("returning SFR Statement for carthesian Coordinates : "
							+ preparedStatementSFDString);
		}
		return preparedStatementSFRString;
	}

	public CombiSearchAlgorithmORSCarthesian(Connection con)
			throws SQLException {
		super(con);
	}
