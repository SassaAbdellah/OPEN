...
public class RideRequestService extends AbstractRestService {
	...
	public String listRideRequests(@Context HttpServletRequest request) {
		...
		LinkedList<RiderUndertakesRideEntity> entities = this
				.lookupRiderUndertakesRideControllerBean().getAllRides();
	...
	public String getRideRequest(@PathParam("id") String id) {
		...
		try {
			...
			RiderUndertakesRideEntity entity = this
					.lookupRiderUndertakesRideControllerBean()
					.getRideByRiderRouteId(idInt);
	...
	public Response addRequest(String json) {
		...
		int updateResult;
		try {
			updateResult = rurcl.addRideRequest(
			// cust_id
					dto.getCustomerId(),
					// starttime_earliest
			...
			// TODO: would be nicer to return a json representation of the error
			// here
			updateResult=-1;
			return Response.status(Response.Status.BAD_REQUEST).build();
	...
	/**
	 * Create a number of ride requests from list of DTOs. (Bulkadd) This is
	 * used for performance test, when we do not want to measure http overhead
	 * when adding multiple requests
	 * 
	 * 
	 * @param request
	 * @return
	...
	public Response addRequests(String json) {
		...
		for (RideRequestDTO dto : dtos) {
			...
			try {
				...
				// TODO: would be nicer to return a decent exception here
				return Response.status(Response.Status.BAD_REQUEST).build();
			}
